...
if [[ $BRANCH_NAME == $MASTER_BRANCH ]]; then
    BUILD_TYPE='master'
elif [[ $BRANCH_NAME == $PREFIX_FEATURE* ]]; then
    BUILD_TYPE='feature'
elif [[ $BRANCH_NAME == $PREFIX_BETA* ]]; then
    BUILD_TYPE='beta'
elif [[ $BRANCH_NAME == $PREFIX_RELEASE* ]]; then
    BUILD_TYPE='release'    
fi

if [ "$BUILD_TYPE" == "" ]; then
    echo "BUILD SKIPPED: Could not determine BUILD_TYPE for $BRANCH_NAME"
    exit 0
fi
...
echo
echo "-----------------------------------------------------------------"
echo "Building $BRANCH_NAME as a $BUILD_TYPE build"
echo "-----------------------------------------------------------------"
echo
