...
class AddRelatedLists(MetadataSingleEntityTransformTask):
    ...
    def _transform_entity(self, metadata, api_name):
        ...
        related_list = self.options.get("related_list")
        ...
        if not existing_related_lists:
            self._create_new_related_list(metadata, api_name, new_related_list_index)

        return metadata

    def _create_new_related_list(self, metadata, api_name, index):
        related_list = self.options.get("related_list")
        self.logger.info(f"Adding Related List {related_list} to {api_name}")

        fields = process_list_arg(self.options.get("fields", []))
