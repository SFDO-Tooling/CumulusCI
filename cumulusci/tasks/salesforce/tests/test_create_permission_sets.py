...
class TestCreatePermissionSet:
    ...
    def test_create_permset(self):
        ...
        responses.add(
            method="POST",
            url=f"{task.org_config.instance_url}/services/data/v{API_VERSION}/sobjects/PermissionSet/",
            status=200,
            json={"id": "0PS3F000000fCNPWA2", "success": True, "errors": []},
        )
        responses.add(
            method="POST",
            url=f"{task.org_config.instance_url}/services/data/v{API_VERSION}/sobjects/PermissionSetAssignment/",
            status=200,
