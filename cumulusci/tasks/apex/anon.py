...
EMAIL_TOKEN = 'mrbelvedere@salesforce.org'
...
class AnonymousApexTask(BaseSalesforceApiTask):
    ...
    def _validate_options(self):
        ...
        if not self.options.get('path') and not self.options.get('apex'):
            raise TaskOptionsError('You must specify either the `path` or `apex` option.')
    ...
    def _run_task(self):
        ...
        if apex_path:
            ...
            try:
                ...
                raise TaskOptionsError("Could not find or read file: {}".format(apex_path))
        else:
    ...
    def _prepare_apex(self, apex):
        # Replace the email string only if it exists
        admin_email = self.project_config.dev_config__admin_email
        if admin_email and EMAIL_TOKEN in apex:
            self.logger.info("Replacing {} with {}".format(EMAIL_TOKEN, admin_email))
            apex = apex.replace(EMAIL_TOKEN, admin_email)
