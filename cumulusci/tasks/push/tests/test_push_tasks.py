...
def test_schedule_push_org_list_get_csv_field_default(tmp_path):
    orgs = tmp_path / ORG_FILE
    orgs.write_text(CSV_FILE_TEXT)
    task = create_task(
        SchedulePushOrgList,
...
def test_schedule_push_org_list_orgs(tmp_path):
    orgs = tmp_path / ORG_FILE
    orgs.write_text(ORG_FILE_TEXT)
    task = create_task(
        SchedulePushOrgList,
...
def test_schedule_push_org_list_no_orgs(tmp_path):
    orgs = tmp_path / ORG_FILE
    orgs.write_text(ORG_FILE_TEXT)
    with pytest.raises(TaskOptionsError):
        create_task(
