...
class GenerateSnowfakeryRecipeYaml(BaseSalesforceApiTask):
    task_docs = """
    Generate snowfakery recipe YAML based on the org schema (not the local metadata).
    """
    task_options = {}

    task_options = {
        "output_file": {"description": "Path to a YAML file to generate."},
        "include": {
            "description": f"Comma-separated list of objects to output or '*' to output all except the default exclude list: {', '.join(EXCLUDE_LIST)}."
        },
        "exclude": {
            "description": "Comma-separated list of objects to add to the exclude list."
        },
    }
    ...
    def _init_options(self, kwargs):
        ...
        if output_file:
            ...
            self.output_file = Path("datasets/generated_recipe.yml")

        with open(self.output_file, "w"):
            pass  # check it is writable

        self.include = process_list_arg(self.options.get("include", "*"))
        self.exclude = process_list_arg(self.options.get("exclude", ""))

    def _run_task(self):
        sobjects = SObjectRelevanceChecker(
            include=["Account", "Contact", "Case"], exclude=[]
        )
        macros = Path(str(self.output_file).replace(".yml", "_macros.yml"))
        with self.org_config.get_org_schema() as schema:
            schema2factory(schema, sobjects, self.output_file, macros)


class SObjectRelevanceChecker:
    def __init__(self, include=["all"], exclude=[]):
        # TODO: not used yet
        self.excluded_sobjects = set(exclude) & (set(EXCLUDE_LIST) - set(include))
        self.included_sobjects = include
...
def schema2factory(schema, relevant_sobjects, out_macro_yml, out_wrapper_yml):
    objects = list(schema.sobjects)
    print("XXX", relevant_sobjects, [obj.name for obj in objects])
    assert "Account" in relevant_sobjects

    chosen_objects = {
        obj.name: obj
        for obj in objects
        if obj.name in relevant_sobjects and ONLY_RELEVANT
    }
    out_macro_versioned = versioned_file(out_macro_yml)
    render_macro_file(chosen_objects, out_macro_versioned)
...
def render_macro_file(objects, out_macro_versioned):

    objs = [render_macro(obj, objects) for objname, obj in objects.items()]
...
def render_macro(obj, objects):
    rc = {}
    rc["macro"] = f"incl_{obj['name']}"
    fields = {field_name: field for field_name, field in obj.fields.items()}
    rc["fields"] = dict(
        render_field(field_name, field, objects)
...
def render_field(name, field, objects):
