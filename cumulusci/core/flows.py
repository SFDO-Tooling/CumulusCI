...
class BaseFlow(object):
    ...
    def __init__(
        ...
        self.project_config = (
            project_config
        )  # a subclass of BaseTaskFlowConfig, tho tasks may expect more than that
    ...
    def _init_flow(self):
        ...
        self.logger.info("---------------------------------------")
        if self.name:
            self.logger.info("Initializing flow: {}".format(self.name))
        else:
            self.logger.info(
        ...
        for step_num, config in list(self.flow_config.steps.items()):
            ...
            if "flow" in config:  # nested flow
                ...
                steps.append(
                    (
                        parsed_step_num,
                        {"step_config": config, "task_config": task_config},
                    )
                )
    ...
    def _run_flow(self, stepnum, step_config):
        flow_options = step_config["step_config"].get("options", {})
    ...
    def _render_task_config(self, task):
        ...
        for option, info in list(task.task_options.items()):
            ...
            if value is None:
                continue
            config.append("  {}: {}".format(option, value))
