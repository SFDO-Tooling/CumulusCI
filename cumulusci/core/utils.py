...
# TODO: move this to cumulusci_yml.py
def merge_cumulus_config(configs: T.Mapping):
    config = merge_configs(configs)
    merge_task_extensions(config.get("tasks", {}))
    return config


def merge_task_extensions(tasks: T.Mapping):
...
def merge_task_extension(
    taskname: str, task: T.MutableMapping, tasks: T.Mapping, stack: set
):
