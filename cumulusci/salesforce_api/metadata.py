...
class BaseMetadataApiCall(object):
    ...
    def _call_mdapi(self, headers, envelope, refresh=None):
        ...
        ), headers=headers, data=auth_envelope.encode('utf-8'))
        faultcode = parseString(
            response.text).getElementsByTagName('faultcode')
    ...
    def _handle_soap_error(self, headers, envelope, refresh, response):
        ...
        faultstring = parseString(
            response.text).getElementsByTagName('faultstring')
        if faultstring:
    ...
    def _process_response_start(self, response):
        if response.status_code == http.client.INTERNAL_SERVER_ERROR:
            raise MetadataApiError('HTTP ERROR {}: {}'.format(response.status_code, response.text), response)
        ...
        if status in ['Succeeded', 'SucceededPartial']:
            ...
            if messages:
                ...
                problems = parseString(
                    response.text).getElementsByTagName('problem')
            ...
            failures = parseString(
                response.text).getElementsByTagName('failures')
