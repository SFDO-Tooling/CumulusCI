...
""",
    ...
    def test_add_files_to_package(self, context):
        with temporary_dir() as path:
            expected = []
...
""",
            ...
            # test
            builder = MetadataPackageZipBuilder(context=context)
    ...
    def test_include_directory(self, context):
        builder = MetadataPackageZipBuilder(context=context)
    ...
    def test_include_file(self, context):
        builder = MetadataPackageZipBuilder(context=context)
    ...
    def test_removes_feature_parameters_from_unlocked_package(self, context):
        with temporary_dir() as path:
...
<Package xmlns="http://soap.sforce.com/2006/04/metadata">
    <types>
        <name>FeatureParameterInteger</name>
            ...
            builder = MetadataPackageZipBuilder(
                path=path, options={"package_type": "Unlocked"}, context=context
            )
            assert (
