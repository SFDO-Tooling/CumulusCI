...
-   `cumulusci/tasks/push/tasks.py:33` — `version_parts = version.split(".")` (no None-guard above). - `cumulusci/tasks/push/tasks.py:283-297` — `_run_task` does not validate `version` before calling `_get_version`. - Test: `/tmp/repro/3/tests/repro_3446_push_qa_no_version.py` passes on v4.10.0.
