...
2. | In `AddFieldsToPageLayout._init_options`, warn when an api_name does not contain `-` (Layout API names always do). The user was on Windows in 2023 - note that the user might also have been hitting a backslash path issue, but the underlying class of bug is the same: api_name format mismatch. ---
