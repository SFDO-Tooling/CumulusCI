...
-   pass2 labels: `bug` **Notes**: Minimal fix at update_profile.py:290-293 - bind `rt_elem = elem.find("recordType")` and check `if rt_elem is not None and rt_elem.text == rt["record_type"]`. Worth a quick scan of sibling code in `_set_record_types` for similar None-deref patterns on optional XML children.
