...
Allowed values are between 1 and 200 for the REST API and 1 and 10,000 for the Bulk API. Note that the semantics of batch sizes differ between the REST API and the Bulk API. In the REST API, the batch size is the actual size of individual transactions. In the Bulk API, the batch size is the maximum record count in a Bulk API batch, which is subject to its own limits, including restrictions on total processing time. Bulk API batches are automatically chunked into transactions by the platform, and the transaction size cannot be controlled.
