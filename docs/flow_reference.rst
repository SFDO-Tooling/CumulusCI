...
.. code-block:: console
    ...
    - install_beta (flow)
        + dependencies (flow)
            ...
            - deploy_pre (task)
        + install_managed_beta (task)
        + config_managed (flow)
            - deploy_post (task)
            - update_admin_profile (task)
        + snapshot_changes (task)
    - run_tests (task)
|

``ci_feature``
^^^^^^^^^^^^^^
Prepares an unmanaged metadata test org and runs Apex tests.
Intended for use against feature branch commits.
